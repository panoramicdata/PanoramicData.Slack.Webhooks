...
{
	[Fact]
	public void ShouldThrowExceptionWhenUrlIsEmpty() => Assert.Throws<ArgumentException>(() => new SlackClient(string.Empty));

	[Fact]
	public void ShouldThrowExceptionIfUrlIsMalformed() => Assert.Throws<ArgumentException>(() => new SlackClient("[/]dodgy_url!@.slack.com"));
	...
	private static SlackMessage GetSlackMessage(
		...
		string iconEmoji = Emoji.Ghost) => new()
		{
			Text = text,
			Channel = channel,
			Username = username,
